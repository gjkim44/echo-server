...
def server(log_buffer=sys.stderr):
    ...
    sock.bind(address)
    sock.listen(1) # 1 person in line can connect to server 
    # ^ try to do it without passing a number
    ...
    try:
        ...
        while True:
            ...
            conn, addr = sock.accept()  # in class
            try:
                print('connection - {0}:{1}'.format(*addr), file=log_buffer)
                # conn.sendall('ready to rec messages'.encode('utf-8'))
